...
#' @param type character. Choose one of the distances: "centroid" (faster, default), where euclidean distance is calculated from feature centroid; "edge", where euclidean distance is calculated from feature edges;
...
#' @param mask object of class sf, sfc, sfg or SpatialPolygons. For higher processing speed of "least-cost" or "commute-time" distances.
...
#' @param geometry_out numeric. Use this argument in case you have selected the "least-cost" or "commute-time".
#' If some spatial geometries are out of the resistance extent, then a buffer zone the large enough to cover these spatial geometries and with this numeric value will be added to the resistance, so that it is possibñe to calculate a cost distance value for the pairs of nodes that involve these geometries.
...
#' @param write character. Choose the output folder if you use name_zona otherwise, place the output path, with the name and extension ".txt".
#' @return Exports a euclidean or cost distance table between pairs of nodes.
#' @references \url{https://www.rdocumentation.org/packages/rgeos/versions/0.3-26/topics/gDistance}\cr
...
distancefile <- function(nodes, id,
  ...
  if (nrow(nodes) > 1) {
    if (is.null(multiple)) {
      if (type %in%  c("centroid", "edge", "hausdorff-edge")){

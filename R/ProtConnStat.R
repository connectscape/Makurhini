...
ProtConnStat <- function(ProtConn, ci= "all", nr=500){
  ...
  ProtConn2 <- as.data.frame(cbind(ProtConn2, cbind(apply(ProtConn,2, sd)),
  ...
  #IC
 if(!is.null(ci)){
  ICBoot <- purrr::map(as.list(ProtConn) , function(n){
      if(length(unique(n)) > 1){
        set.seed(626)
        bootcorr <- boot::boot(n, mean.fun, R = nr)
        err2 <- tryCatch(boot::boot.ci(boot.out = bootcorr, type = c("norm", "basic", "perc", "bca")), error = function(err)err)
        if (inherits(err2, "error")){
          ...
          err2 <- purrr::map(as.list(ci), function(x){

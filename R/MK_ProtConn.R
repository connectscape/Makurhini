      ...
      if (nrow(nodes.2) > 1 ){
        if(area2_sort[[1]] < sum(unit_convert(as.numeric(st_area(region)), "m2", area_unit))){
          ...
          if(length(bound_nodes2$IDTemp) == 1){
            distance_base$Distance[which(distance_base$From == as.numeric(bound_nodes2$IDTemp))] <- 0
            distance_base$Distance[which(distance_base$To == as.numeric(bound_nodes2$IDTemp))] <- 0
          }
        }
      }

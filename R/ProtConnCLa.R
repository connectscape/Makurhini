...
#' @param region object of class sf, sfc, sfg or SpatialPolygons. Ecoregion shapefile.
#' @param thintersect numeric. Threshold of intersection in percentage allowed to select or not a target geometry. Example, if equal to 90 then a nodes will be selected only if the intersection between the node and the region is >= 90 percentage. If NULL, then the nodes will be selected withou
...
ProtConnCLa <- function(nodes, region, thintersect = NULL,
  ...
  over_sf <- function(x, y) {
    if(class(x)[1] == "SpatialPolygonsDataFrame") {
      x <- sf::st_as_sf(x) %>% sf::st_cast("POLYGON")
    }
    if(class(y)[1] == "SpatialPolygonsDataFrame") {
      y <- sf::st_as_sf(y) %>% sf::st_cast("POLYGON")
    }
    over_result <- sapply(sf::st_intersects(x %>% sf::st_cast("POLYGON"), sf::st_geometry(y)), function(z) if (length(z)==0) NA_integer_ else z[1])
    return(over_result)
  }
  ...
  region <- sf::st_as_sf(x = region) %>% st_zm(.) %>%
    sf::st_buffer(., dist = 0) %>% sf::st_cast("POLYGON")
  nodes <- sf::st_as_sf(x = nodes) %>% st_zm(.) %>% sf::st_cast("POLYGON")

  #Select PAs
  select_distance <- max(c(transboundary, distance_thresholds)) * 2

  #Simplify region
  if(mapview::npts(x = region) > 100){
    region_1<- tryCatch(sf::st_simplify(x = region, dTolerance = 10000, preserveTopology = TRUE),
                        error = function(err)err)#First selection to reduce the extent of work and dissolve polygones

    if (inherits(region_1, "error")){
      region_1 <- sf::st_buffer(x = region, dist = select_distance)
      region_1 <- sf::st_buffer(x = region_1, dist = 0) %>% sf::st_cast("POLYGON")
    } else {
      region_1 <- sf::st_buffer(x = region_1, dist = select_distance)
    }
  } else {
    region_1 <- sf::st_buffer(x = region, dist = select_distance)
  }
  ...
  if (inherits(select_PA, "error")){
    nodes <- sf::st_buffer(x = nodes, dist = 0)
    select_PA <- over_sf(x = nodes, y = region_1)
    nodes.1 <- sf::st_as_sf(x = nodes[!is.na(select_PA),]) %>% sf::st_cast("POLYGON")
  } else {
    nodes.1 <- sf::st_as_sf(x = nodes[!is.na(select_PA),]) %>% sf::st_cast("POLYGON")
  }
  ...
  if (is.null(transboundary) | length(transboundary) == 1){
    ...
    if(nrow(nodes.1) >= 1){
      ...
      nodes.1$dis <- 1
      TopologError <- tryCatch(rmapshaper::ms_dissolve(input = nodes.1, field = "dis")%>%
                                 sf::st_cast("POLYGON"), error = function(err)err)

      if (inherits(TopologError, "error")){
        nodes.1 <- sf::st_buffer(x = nodes.1, dist = 0) %>%
          sf::st_cast("POLYGON")
        nodes.2 <- rmapshaper::ms_dissolve(input = nodes.1, field = "dis") %>%
          sf::st_cast("POLYGON")
      } else {
        nodes.2 <- TopologError
      }

      TopologError <- tryCatch(nodes.2 %>% sf::st_cast("POLYGON"), error = function(err)err)
      if (inherits(TopologError, "error")){
        nodes.2 <- sf::st_buffer(x = nodes.2, dist = 0)%>%
          sf::st_cast("POLYGON")
        nodes.2$dis <- NULL
      } else {
        nodes.2 <- TopologError
      ...
      if(mapview::npts(region) > 100){
        region_2 <- rmapshaper::ms_simplify(input = region, keep = 0.2, keep_shapes = TRUE, explode = TRUE) %>% rmapshaper::ms_dissolve(.)
      } else {
        region_2 <- region
      }

      ##Select by location and %intersection the dissolve PAs -> nodes.2
      ...
      if(nrow(nodes.2) > 0){
        nodes.2 <- rmapshaper::ms_dissolve(nodes.2)%>% sf::st_buffer(., dist = 0) %>%
          sf::st_cast("POLYGON")
        nodes.2 <- SelectbyLoc(target = nodes.2, sourcelyr = region_2,
                               selreg = "M2", thintersect = thintersect,
                               transboundary = transboundary, SAGA = SAGA)
      ...
      if(nrow(nodes.2) == 1){
        ...
        nodes.2$Area2 <- as.numeric(sf::st_area(nodes.2)) * 0.0001
    ...
    if (nrow(nodes.2) > 1 ){
      if (area2_sort[[1]] < sum(as.numeric(sf::st_area(region)) * 0.0001)){
        nodes.2$IDTemp <- 1:nrow(nodes.2)

        bound_nodes <- nodes.2
        ##bound nodes
        region2 <- region %>% sf::st_cast("POLYGON")
        ...
        if (!is.null(distance$resistance)) {
          ...
          resistance_protconn$crop(as(sf::st_buffer(mask, dist = distance_thresholds), 'Spatial'))
        ...
        if (inherits(distance_base, "error")){
          bound_nodes <- sf::st_buffer(bound_nodes, dist = 0)
          distance_base <-  tryCatch(distancefile(nodes = bound_nodes,  id = "IDTemp", type = distance$type,
                                                  tolerance = distance$tolerance, resistance = resistance_protconn,
                                                  threshold = distance$threshold, geometry_out = distance$geometry_out,
                                                  distance_unit = distance$distance_unit,
    ...
    result_protconn <- tryCatch(purrr::map(as.list(1:length(distance_thresholds)), function(i){
      nodes.2 <- nodes_base
      #If there are protected areas
      if (nrow(nodes.2) > 1 ){
        if (area2_sort[[1]] < sum(as.numeric(sf::st_area(region)) * 0.0001)){
          #Calculate Node Attribuite
          if (attribute == "Intersected area"){
            ...
            r.raster <- raster::raster(nodes.1, res= res_attribute)
            r.raster <- fasterize::fasterize(nodes.1, r.raster, field = attribute, background = 0)
          ...
          if (is.null(LA) & isTRUE(attribute %in% c("Area", "Intersected area"))){
            LA <- sum(as.numeric(sf::st_area(region)) * 0.0001)
          } else {
          ...
          result.1 <- tryCatch(EstConefor(nodeFile = "nodes.txt", connectionFile = "Dist.txt",
                                          ...
                                          index = "PC", thdist = distance_thresholds[i],
          ...
          #dPC merge
          if(isTRUE(dPC)){
            if(!is.infinite(unique(result.1[["node_importances"]][[4]]))){
              dPC.2 <- merge_conefor(data = result.1[["node_importances"]],
          ...
          data <- as.data.frame(result.1$overall_indices)
          ...
          #ProtConn Indicators
          DataProtconn$Prot <- 100 * (DataProtconn$a / sum(as.numeric(sf::st_area(region)) * 0.0001))#Percentage of the study region covered by PAs
          ...
          if(isFALSE(unique(nodes.2$transboundary) == 1)){
            ...
            if (nrow(nodes.2) > 1){
              ...
              result.2 <- as.data.frame(result.2$overall_indices)
              ...
              if (inherits(TopologError, "error")){
                nodes.1 <- sf::st_buffer(nodes.1, dist = 0) %>% sf::st_cast("POLYGON")
                nodes.2 <- sf::st_buffer(nodes.2, dist = 0) %>% sf::st_cast("POLYGON")
                TopologError <- tryCatch(over_sf(nodes.1, nodes.2), error = function(err)err)
                nodes.3 <- nodes.1[!is.na(TopologError),]
              }
              ...
              if(nrow(nodes.3) >= 1){
                nodes.3$A <- as.numeric(sf::st_area(nodes.3, by_element = TRUE) * 0.0001)
                pi2 <- tryCatch(sf::st_intersection(nodes.3, region_2), error = function(err)err)

                if (inherits(pi2, "error")){
                  nodes.3 <- sf::st_buffer(nodes.3, dist = 0)
                  region_2 <-  sf::st_buffer(region_2, dist = 0)
                  pi2 <- sf::st_intersection(nodes.3, region_2)
                }
          ...
          if (nrow(bound_nodes) > 1){
            ...
            result.3 <- as.data.frame(result.3$overall_indices)
      ...
      if (nrow(nodes.2) > 0 & isTRUE(plot)){
        ...
        Data_plot_1$Indicator <- factor(Data_plot_1$Indicator, levels = c(Data_plot_1$Indicator[1], Data_plot_1$Indicator[2], Data_plot_1$Indicator[3]))
        ...
        if(nrow(Data_plot_1) > 1){
          plot_protconn1 <- ggplot2::ggplot(Data_plot_1, ggplot2::aes(x = name, y = Values, fill = name)) +
            ...
            ggplot2::scale_fill_manual(values = Data_plot_1$col) +
        ...
        if(nrow(Data_plot_2) > 1){
          plot_protconn2 <- ggplot2::ggplot(Data_plot_2, ggplot2::aes(x = name, y = Values, fill = name)) +
            ggplot2::geom_bar(position = ggplot2::position_dodge(), colour = "black", stat = "identity", show.legend = FALSE, size = 0.2) +
            ...
            ggplot2::theme(plot.title = ggplot2::element_text(color = "#252525", size = ggplot2::rel(1.4), hjust = 0.45, face = "bold"),
                           ...
                           legend.text = ggplot2::element_text(colour = "#252525", size = ggplot2::rel(1.2)),
                           axis.text= ggplot2::element_text(colour = "#525252", size = ggplot2::rel(1)))+
            ggplot2::scale_fill_manual(values = Data_plot_2$col)
          plot_protconn[[2]] <- plot_protconn2
        }
        plot_protconn <- plyr::compact(plot_protconn)

        if(length(plot_protconn) == 2){
          figure <- ggpubr::ggarrange(plot_protconn[[1]], plot_protconn[[2]],
                                       ncol = 1, nrow = 2)
      ...
      DataProtconn_4 <- formattable::formattable(DataProtconn_3, align = c("l","c"),
                                                 list(`Index` = formattable::formatter("span", style = ~ formattable::style(color = "#636363", font.weight = "bold")),
                                                      `ProtConn indicator` = formattable::formatter("span", style = ~ formattable::style(color = "#636363", font.weight = "bold")),
                                                      `Percentage` = formattable::color_tile("white", "orange")))
      ...
      if(!is.null(write)){
        ...
        if(isTRUE(dPC)){
          sf::write_sf(result[[3]], paste0(write,"_d", distance_thresholds[i], "_dPC.shp"), delete_layer = TRUE)
        }
      }
      result2 <- plyr::compact(result)

      if (isTRUE(intern) & length(distance_thresholds)>1){
        pb$tick()$print()
      }
      return(result2)}), error = function(err)err)
    ...
    pb <- dplyr::progress_estimated(length(transboundary), 0)
    result_protconn <-  tryCatch(purrr::map(as.list(transboundary), function(x){
      pb2 <- dplyr::progress_estimated(length(distance_thresholds), 0)
      if(is.null(x)){
        x.2 = 100
      } else {
        x.2 = x
      ...
      if(nrow(nodes.1) >= 1){
        nodes.1$IDTemp <- 1:nrow(nodes.1)
        nodes.1$dis <- 1
        TopologError <- tryCatch(rmapshaper::ms_dissolve(nodes.1, field = "dis")%>% sf::st_cast("POLYGON"), error = function(err)err)

        if (inherits(TopologError, "error")){
          nodes.1 <- sf::st_buffer(nodes.1, dist = 0) %>% sf::st_cast("POLYGON")
          nodes.2 <- rmapshaper::ms_dissolve(nodes.1, field = "dis") %>% sf::st_cast("POLYGON")
        } else {
          nodes.2 <- TopologError
        }
        TopologError <- tryCatch(nodes.2 %>% sf::st_cast("POLYGON"), error = function(err)err)
        ...
        if(mapview::npts(region) > 100){
          region_2 <-rmapshaper::ms_simplify(region, keep = 0.2, keep_shapes = TRUE, explode = TRUE) %>% rmapshaper::ms_dissolve(.)
        } else {
          region_2 <- region
        }
        ...
        if(nrow(nodes.2) > 0){
          nodes.2 <- rmapshaper::ms_dissolve(nodes.2)%>% sf::st_buffer(., dist = 0) %>% sf::st_cast("POLYGON")
          nodes.2 <- SelectbyLoc(target = nodes.2, sourcelyr = region_2,
                                 selreg = "M2", thintersect = thintersect,
                                 transboundary = x.2, SAGA = SAGA)
        }
      ...
      result_protconn_2 <- tryCatch(purrr::map(as.list(1:length(distance_thresholds)), function(i){
        nodes.2 <- nodes_base
        #If there are protected areas
        if (nrow(nodes.2) > 1 ){
          if (area2_sort[[1]] < sum(as.numeric(sf::st_area(region)) * 0.0001)){
            ...
            if (nrow(bound_nodes) > 1){
              ...
              result.3 <- as.data.frame(result.3$overall_indices)
              ECA3 <- as.data.frame(cbind(ECA = result.3[2,2], PC = result.3[3,2]))
              DataProtconn$ProConn_design <- (100*(ECA3$ECA/DataProtconn$LA)) - DataProtconn$ProtConn
              DataProtconn$ProConn_Bound<- DataProtconn$Prot - DataProtconn$ProConn_design
            }
          ...
          if(nrow(Data_plot_1) > 1){
            plot_protconn1 <- ggplot2::ggplot(Data_plot_1, ggplot2::aes(x = name, y = Values, fill = name)) +
              ...
              ggplot2::theme(plot.title = ggplot2::element_text(color = "#252525", size = ggplot2::rel(1.4), hjust = 0.5, face = "bold"),
                             ...
                             legend.title= ggplot2::element_blank(),
                             legend.text = ggplot2::element_text(colour = "#252525", size = ggplot2::rel(1.2)),
                             axis.text= ggplot2::element_text(colour = "#525252", size = rel(1)))+
              ggplot2::scale_fill_manual(values = Data_plot_1$col) +
              ggplot2::geom_hline(ggplot2::aes(yintercept = 17, linetype = "Aichi Target (17%)"), colour = 'black', size = 1.2) +
              ggplot2::scale_linetype_manual(name = " Aichi Target", values = c(2, 2),
                                             guide = ggplot2::guide_legend(override.aes = list(color = c("black"), size = 0.8)))
      ...
      return(result_protconn_2)}), error = function(err)err)

...
MK_ProtConnMult <- function(nodes, regions, thintersect = NULL,
  ...
  if(isFALSE(parallel)){
    ...
    x=NULL
    protconn_result <- foreach(x = iter(1:length(regions$ID_Temp))) %dopar%
      {
    ...
    Ecoreglist <- foreach(x = iter(protconn), .errorhandling = 'pass') %dopar%
      {
      n <- as.vector(x[[1]][[3]])
    ...
    if (!is.null(attr(warnErrList(Ecoreglist), "warningMsg")[[1]])) {
      setwd(ttt.2)
      stop(Ecoreglist)
    }

    ECAlist <- foreach(x = iter(protconn), .errorhandling = 'pass') %dopar%
      {
      n <- as.vector(x[[1]][[1]])
    ...
    if (!is.null(attr(warnErrList(ECAlist), "warningMsg")[[1]])) {
      setwd(ttt.2)
      stop(ECAlist)
    }
    ...
    protconn_result <- tryCatch(future_map(as.list(1:length(regions$ID_Temp)), function(x){
      ...
      Ecoreglist <- tryCatch(future_map(protconn, function(x){
        n <- as.vector(x[[1]][[3]])
        x2 <- t(x[[1]][[4]]) %>% as.data.frame()
        colnames(x2) <- n
      ...
      ECAlist <- tryCatch(future_map(protconn, function(x){
        n <- as.vector(x[[1]][[1]])
        x2 <- t(x[[1]][[2]]) %>% as.data.frame()
        colnames(x2) <- n
        return(x2)}), error = function(err)err)
  ...
  i=NULL
  j=NULL
  data_1 <- foreach(i = iter(1:length(distance_thresholds)), .errorhandling = 'pass') %dopar%{
    data_2 <- foreach(j = iter(1:length(regions$ID_Temp))) %do%
      {
        Ecoreglist[[j]][[i]]
      }
    ...
    ECA_2 <- foreach(j = iter(1:length(regions$ID_Temp))) %do%
      {
        ECAlist[[j]][[i]]
      }
    ...
    if(isTRUE(plot)){
        ...
        if(nrow(dacc) > 1){
          ...
          dacc[6:7] <- apply(dacc[6:7], 2, function(x){
            ...
            x[which(x <0)]<- 0
            return(x)})
          plot_protconn2 <- ggplot(dacc, aes(x = dacc$name, y = dacc$Values, fill = dacc$name)) +
            geom_bar(position = position_dodge(), colour = "black", stat = "identity", show.legend = FALSE, size = 0.2) +
            geom_errorbar(position = position_dodge(), width = 0.2, aes(ymin = min, ymax = max)) +
  ...
  if (!is.null(attr(warnErrList(data_1), "warningMsg")[[1]])) {
    setwd(ttt.2)
    stop(warnErrList(data_1))
  }
  setwd(ttt.2)
